...
import invite_prevalidate_discourse_email from './slack-invite';
...
export default function slackin({
  ...
  if (!discourseBaseUrl) throw new Error('Must provide an `disc-base-url`.');
  if (!discourseApiKey) throw new Error('Must provide an `disc-api-key`.');
  if (!discourseApiUsername) throw new Error('Must provide an `disc-api-username`.');
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    invite_prevalidate_discourse_email({ token, org, email, channel: chanId, disc_base_url: discourseBaseUrl, disc_api_key: discourseApiKey, disc_api_username: discourseApiUsername }, err => {
      if (err) {
        return res
        .status(400)
